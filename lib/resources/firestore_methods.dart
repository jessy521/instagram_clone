...
class FirestoreMethods {
  ...
  Future<void> likePost(String postId, String uid, List likes) async {
    try {
      if (likes.contains(uid)) {
        await _firebaseStore.collection('posts').doc(postId).update({
  ...
  Future<void> delPost(String postId) async {
    try {
      await _firebaseStore.collection('posts').doc(postId).delete();
    } catch (e) {
      print(e.toString());
    }
  }
  ...
  Future<void> followUser(String uid, String followId) async {
    try {
      DocumentSnapshot snap =
          await _firebaseStore.collection('users').doc(uid).get();
      List following = (snap.data()! as dynamic)['following'];

      if (following.contains(followId)) {
        await _firebaseStore.collection('users').doc(followId).update({
